...
export const analyseResume = TryCatch(async(req: AuthenticatedRequest, res) => {
...
export const jobMatcher = TryCatch(async(req:AuthenticatedRequest, res) => {
    ...
    if (mode==="manual" && (!skills.length || !experience?.trim())) return res.status(400).json({message:"Skills and experience are required"})
...
export const generateInterview = TryCatch(async(req:AuthenticatedRequest, res) => {
    const {mode,round, skills, experience, pdfBase64} = req.body

    if (!mode || !round) return res.status(400).json({message:"Mode and round are required"})
    if (mode==="manual" && (!skills.length || !experience?.trim())) return res.status(400).json({message:"Skills and experience are required"})
    ...
    const parts: any[] = [{text: generateInterviewPrompt(mode,round, skills, experience)}]
...
export const buildResume = TryCatch(async(req: AuthenticatedRequest, res) => {
    ...
    if (!mode) return res.status(400).json({message:"Mode is required"})
    if (mode==="manual" && !formData ) return res.status(400).json({message:"fromData is required"})
